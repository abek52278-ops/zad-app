...
object SaBankParser {
    ...
    val moneyKeywords = listOf(
        ...
        "paiement", "achat", "solde", "virement", "retrait", "carte bancaire", "montant", "débit", "crédit",
            // ── عملات عالمية (مضافة 2026-09-06) ──
        // القايمة كانت فيها ١٩ عملة إقليمية وصفر عملة عالمية. مشتريات الإنترنت
        // (أمازون.كوم، نتفليكس، Booking) بتتحاسب بالدولار حتى لعملاء مصر والخليج،
        // وحسابات الدولار شائعة في لبنان والعراق والسودان. "$45.00 - Netflix" و
        // "USD 120.00 - Amazon.com" كانوا بيسقطوا من البوابة بالكامل.
        //
        // ⚠️ الإضافة هنا لوحدها ماكانتش تنفع: SaBankParser.CUR كان فيه نفس الثغرة،
        // فالإشعار كان هيعدّي فحص الكلمات وبعدين extractAmount يرجّع null فيسقط تاني.
        // التلات مواضع (هنا + CUR + unambiguousCurrencyTokens) لازم يتحركوا مع بعض.
        "USD", "\$", "EUR", "€", "GBP", "£", "دولار", "يورو",
    )

    /** الإشعار بيتكلم عن فلوس؟ — نص البوابة، والنص التاني هو وجود مبلغ فعلي. */
    fun mentionsMoney(text: String): Boolean =
        moneyKeywords.any { text.contains(it, ignoreCase = true) }
    ...
    private const val CUR = """(?:ر\.س|رس|ريال|SAR|SR|""" +
        ...
        """TND|د\.ت|""" +
        """DZD|د\.ج|""" +
        // عملات عالمية — مشتريات الإنترنت بتتحاسب بيها حتى لعملاء مصر والخليج.
        // \${'$'} متهرّبة: علامة الدولار مرساة في الريجيكس لو ساب من غير هروب.
        """USD|\${'$'}|EUR|€|GBP|£|دولار|يورو|""" +
        """دينار|درهم)"""
