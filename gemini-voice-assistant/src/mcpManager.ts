...
export class MCPManager {
  ...
  private registeredTools: Map<string, { serverName: string; originalName: string }> = new Map();

  /**
   * Register standard sample tools for instant out-of-the-box functionality
   */
  private fallbackTools: GeminiFunctionDeclaration[] = [
    {
   ...
   */
  public async connectStdioServer(serverName: string, command: string, args: string[] = []): Promise<void> {
    try {
  ...
  public getGeminiFunctionDeclarations(): GeminiFunctionDeclaration[] {
    const declarations: GeminiFunctionDeclaration[] = [...this.fallbackTools];
   ...
   */
  public async executeToolCall(name: string, args: Record<string, any>): Promise<Record<string, any>> {
