-- اكتشاف التكرار: مفيش تصنيف دلالي لنوع الخدمة هنا عمدًا — category عمود نص حر (يتحدد
-- من الموديل وقت add_subscription أو من الشات)، مفيش enum ولا قايمة مضبوطة، فمقارنته
-- بمعنى "الاتنين خدمة فيديو" كانت هتبقى تخمين زي subscription price change. المطابقة
-- هنا حرفية بس (نفس النص بعد تطبيع بسيط) — "عندك ٢ اشتراك متصنّفين نفس التصنيف" حقيقة
-- مقاسة، مش استنتاج عن نوع الخدمة.
...
BEGIN
  ...
  SELECT v_out || CASE WHEN count(*) >= 2 THEN jsonb_build_array(jsonb_build_object(
      'domain', 'inventory', 'kind', 'waste_summary',
      'text', 'الشهر ده اتهدرلك ' || count(*) || ' صنف بعد ما انتهت صلاحيته من غير ما يتاكل — آخرهم ' ||
              (array_agg(item_name order by logged_at desc))[1],
      'severity', 'low',
      'data', jsonb_build_object('count', count(*), 'items', array_agg(distinct item_name))
    )) ELSE '[]'::jsonb END
    INTO v_out
    FROM zad_waste_log
   WHERE user_id = p_user AND logged_at >= now() - interval '30 days';
  ...
  -- جديد: نفس التصنيف على ٢+ اشتراك نشط — مطابقة نص حرفية (lower+trim)، مش تخمين نوع خدمة.
  SELECT v_out || coalesce(jsonb_agg(jsonb_build_object(
      'domain', 'subscriptions', 'kind', 'possible_duplicate_category',
      'text', 'عندك ' || grp.cnt || ' اشتراكات متصنّفة "' || grp.cat || '": ' || grp.titles ||
              ' — محتاج الاتنين فعلاً؟',
      'severity', 'low',
      'data', jsonb_build_object('category', grp.cat, 'count', grp.cnt, 'titles', grp.titles_arr)
    )), '[]'::jsonb)
    INTO v_out
    FROM (
      SELECT lower(btrim(category)) AS cat, count(*) AS cnt,
             string_agg(title, '، ' order by title) AS titles,
             array_agg(title order by title) AS titles_arr
        FROM zad_subscriptions
       WHERE user_id = p_user AND is_active AND category IS NOT NULL AND btrim(category) <> ''
