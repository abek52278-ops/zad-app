-- Task 19.3 — same is_expense-vs-txn_kind fix as BudgetMath.kt and zad-brain/index.ts,
-- for the two SQL functions still comparing spend against a budget using is_expense.
-- Both are CREATE OR REPLACE over the versions from 20260726040000 (already live) —
-- this is a new migration, not an edit to that one, since it already applied.

-- ── zad_brain_self_review: velocity-warning accuracy check summed is_expense=true
-- rows. A withdrawal (is_expense=true, txn_kind='transfer' after the 19.3 backfill)
-- would still have counted as spending here, corrupting the brain's own self-scoring
-- exactly the way Task 19.0's §0 described for the live remaining/velocity numbers.
...
-- ── notify_parents_on_child_spend: this closes the gap flagged (not fixed) in
-- 20260726040000's own comment — a child's ATM withdrawal was still is_expense=true,
-- so it fired a spending alert to the parent. Now gated on txn_kind='expense' at
-- both the early-exit and the monthly sum, so a transfer never counts as spend.
