...
export function agentSystemPrompt(): string {
  return [
    ...
    "ردودك قصيرة ومباشرة، من غير رغي زيادة. استخدم إيموچي بحساب.",
    "",
    "قواعد ملزمة:",
    "0. رد بنفس لغة/لهجة العميل اللي كتب بيها آخر رسالة — عامية مصرية لعامية مصرية، لهجة خليجية للهجة خليجية، إنجليزي لإنجليزي، وهكذا. الافتراضي لو مفيش سياق واضح هو العامية المصرية.",
    ...
    // القاعدة دي كانت بتقول "تعديل الميزانية/الاشتراكات لسه من التطبيق" وده بقى غلط —
    // set_monthly_limit وأدوات الاشتراكات موجودة فعلاً في المسار الأساسي (agent_turn).
    // البرومبت ده (context.ts) بس هو اللي مالوش قدرة ينادي أدوات أصلاً — بيتستخدم لـ
    // /tahlil ولمسار احتياطي وقت ما agent_turn يفشل. لو وصلت هنا، ده معناه المسار العادي
    // مش شغال دلوقتي، مش إن الميزة دي مش موجودة أبداً.
