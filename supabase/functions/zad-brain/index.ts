...
function getAssistantName(snap: any): { nameAr: string; nameEn: string } {
  const g = String(snap?.user?.gender || snap?.profile?.gender || snap?.gender || "").toLowerCase();
  const isFemale = g === "female" || g === "f" || g.includes("أنثى") || g.includes("انثى") || g.includes("بنت") || g.includes("ام");
  if (isFemale) {
    return { nameAr: "zada ai (زادا AI)", nameEn: "zada ai" };
  }
