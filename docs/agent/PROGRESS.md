...
## 2026-08-16 — زاد بقى دفتر حسابات (الرصيد بدل السقف)

`الرصيد = الرصيد الابتدائي + كل الدخل - كل المصروف`. مفيش سقف ميزانية في المنتج بعد
النهاردة، ومفيش دخل مستنّي موافقة عشان يتحسب.

### السبب
العميل كان شايف **-500** وراتب 10,000 واصل فعلاً وموجود في `income`. مكانش في بق في
الحساب — دي كانت القاعدة شغّالة زي ما اتكتبت: `remaining = monthly_limit - spent +
income_allocated`، و`counts_toward_budget` افتراضيها `null` و`null` مستبعد. يعني الراتب
كان لازم حد يسأل العميل عنه الأول عشان يحرّك الرقم. القاعدة دي كانت **صح** وهي سقف صرف
(إيداع 20,000 مش معناه 20,000 مصاريف بيت زيادة — migration 20260815133417)، وبقت غلط أول
ما الرقم بقى رصيد.

### اللي اتعمل
- **`20260816010000_ledger_balance.sql`** — `zad_budget_state` (الـ wrapper، مش الـ 180
  سطر بتوع `_legacy`) بقى بيحسب `balance = opening + income - spent` ويدهس بيه
  `remaining`/`available`/`velocity`/`threat`. `monthly_limit` **اتساب باسمه** ومعناه بقى
  "الرصيد الابتدائي للدورة" — تغيير الاسم كان هيلمس ~٤٠ نقطة استدعاء من غير مكسب سلوكي؛
  `opening_balance` و`balance` بيتبعتوا جنبه بالأسماء الصادقة.
- **auto-approve**: backfill لكل صف دخل `counts_toward_budget is null` → `true`،
  والعمود بقى `default true`. الرقم مابيستناش سؤال. العمود اتساب لأن "العميل قال ده مش
  للبيت" لسه معلومة العقل بيستخدمها في النصيحة، حتى لما مابقاش لها أثر في الرقم.
- **`BudgetMath.kt`** — المرآة الأوفلاين اتظبطت على نفس القاعدة بالحرف: `balanceInCycle`
  جديدة (بترجع رقم دايماً)، `remainingInCycle` بقت هي هي بس بتحافظ على اتفاقية
  null-معناها-متحددش، و`velocityInCycle` بقت بتقيس السرعة على `opening + income` مش على
  سقف مابقاش موجود.
- **خصم سريع** — زرار ثابت على الرئيسية (ExtendedFAB)، حقل واحد، بيسجّل مصروف
  `isVerified=true` تحت "أخرى" على طول من غير ما يعدّي على العقل. الفورم الكامل اتنقل
  لزرار صغير فوقه.
- **تعديل يدوي للرصيد** — العميل بيكتب الرقم اللي المفروض يشوفه. أول مرة = ده الرصيد
  الابتدائي؛ بعد كده بتتسجّل **معاملة تصحيح** بالفرق. عمداً مش تعديل للرصيد الابتدائي:
  تصحيح لتحت على حساب دخله وصل بيدّي رصيد ابتدائي بالسالب، وكل قارئ في النظام بيقرا
  `monthly_limit <= 0` على إنها "مفيش رصيد" — يعني التصحيح كان هيمسح الكارت بدل ما يصلّحه.
  ومعاملة التصحيح بتفضل سطر ظاهر ينفع يتمسح، مش رقم اتغيّر في الخفا (نفس مبدأ
  `BalanceAnchor`).
- **البوت والعقل** — الأرقام بتتصحح لوحدها (نفس الـ RPC)، بس **الكلام لأ**: "المتبقي من
  السقف" بقت هتوصف فلوس العميل على إنها بدل صرف باقي. الليبلات في `telegram.ts`/
  `context.ts` بقت "رصيدك"/"الرصيد الابتدائي للدورة"، وقاعدة ٩ في برومبت البوت بقت تقول
  المعادلة صراحة. `set_monthly_limit` في `zad-brain` بقى وصفه "اظبط رصيد العميل".
  `allocate_income` بقت بتقول الحقيقة: الرقم مابيتغيّرش، بتسجّل نية بس.
- **Pexels بدل Unsplash** لصور الأكل (اتوسّع بعدين لكل مكوّنات الأكل — الفقرة اللي تحت) (`PEXELS_API_KEY`, ترويسة `Authorization` **خام**
  من غير بادئة). أهم مكسب: Unsplash كانت بترجّع `[]` على أي عربي، فالوصفة كانت بتخسر
  صورتها لما النموذج ينسى `image_keyword_en`؛ دلوقتي اسم الوصفة العربي احتياطي حقيقي.
  مفتاح الكاش بقى `meal_image_v2:` عشان روابط Unsplash القديمة تموت بالـ TTL لوحدها.

### لسه مفتوح
- البوت مابياخدش "صرفت ٥٠ جنيه مياه" ويخصمها فوراً، ومفيش inline keyboards للتأكيد —
  ده كان النص التاني من طلب العميل واتأجّل عن قصد.
- الشيف/الأمازون/كارت وكيل زاد/اللوكيشن — ما اتلمسوش في الجلسة دي.

---

## 2026-08-16 (تكملة) — Pexels في كل مكوّنات الأكل، مش في الأكشن بس

النص الأول غيّر مزوّد الصور جوه `meal_suggestions`/`recipe_details` بس. المشكلة اللي فضلت:
**شاشة تفاصيل الوصفة مكانش فيها صورة شبكة من الأساس** — تدرّج لوني وإيموجي، فاضلين من
وقت ما Unsplash قفلت الـ hotlink. يعني نفس الوصفة يبقى ليها صورة في كارت الشيف الصغير
ومالهاش في الشاشة اللي المفروض تكون أوضح منه.

### اللي اتعمل
- **`PexelsRepo.kt`** — بحث صورة واحد للكلاينت، بكاش محلي دائم (SharedPreferences، نفس
  نمط `AiLocalCache`). الكاش **مش مقسوم على المستخدم** عن قصد: صورة "كشري" مش بيانات حد،
  وتقسيمها كان هيخلي كل حساب على الجهاز يدفع نفس النداء تاني. **والغياب متخزّن زي
  الوجود** (TTL أقصر: ١٢ ساعة مقابل ٧ أيام) — من غير كده، أكلة Pexels مالهاش صورة ليها
  كانت هتاخد نداء شبكة مع كل recomposition.
- **مسارين للمفتاح.** `BuildConfig.PEXELS_API_KEY` لو متجمّع (زي ما اتطلب)، وإلا أكشن
  `pexels_image` الجديد في `zad-core-intelligence`. الاحتياطي مش تزويق: قاعدة المشروع
  المكتوبة في `LocationIqRepo` إن المفتاح سر سيرفر فقط "أبداً في الـ APK، عشان محدش يقدر
  يفك التطبيق ويستهلك حصة الحساب المجانية". المسار السيرفري بيخلي شحن APK من غير مفتاح
  خيار شغّال بالكامل. قيمة `.env.example` الافتراضية بتتقرا كـ"مفيش مفتاح" — من غير كده
  كل صورة في بناء CI كانت هتاخد رحلة كاملة عشان ترجع 401.
- **`FoodImage.kt`** — مكوّن واحد بيحسم الترتيب: الرابط اللي جه مع الوصفة، وإلا بحث
  Pexels بالاسم العربي، وإلا البديل. بيتعامل مع **فشل تحميل الرابط** كمان، مش غيابه بس:
  Coil بياخد `error` كـPainter مش composable، فرابط ميت كان هيدّي مربع فاضي بدل الأيقونة.
  ورابط وصفة ميت بيسقط على بحث Pexels بدل ما يبقى حكم نهائي.
- البديل بيتعرض **أثناء التحميل** كمان، مش سبينر — صورة الأكل تزيين مش محتوى.
- **`PexelsRepoTest`** — شكل رد Pexels (`photos[].src.landscape`) مختلف عن Unsplash
  (`results[].urls.regular`)، وعدم التطابق مابيرميش استثناء: بيرجع null، وnull مش فارقة
  عن "مفيش صورة للأكلة دي" — يعني الكروت كانت هتفضل على الأيقونة للأبد وتبان مشكلة
  محتوى مش مشكلة قراءة.

### محتاج
`PEXELS_API_KEY` كسر مشروع في Supabase (للمسار السيرفري)، و/أو في `.env` محلي (للمسار
المباشر).

---

## الكارت الأخضر يبقى رقم واحد، والرقم يبدأ من ساعة ما العميل يقوله (2026-08-16)

تعليمة مباشرة من العميل، مش تاسك مرقّم. النص: "عاوز كرت الميزانية فقط الرقم اللي العميل
يدخله يبتدي يحسب على أساسه كل شيء، مليش دعوة بمصاريفه القديمة ... بعد كده النوتيفيكيشن
ليسننج رصدت أي صرف يتخصم من الكارت الأخضر، ويقراها عقل الايجنت يحلل يشوف سحب ولا إيداع
ويبعتلي عن طريق البوت، أقوله أيوة أو لا، ويسجل ويعدل الكارت الأخضر".

### تلات حاجات كانت غلط، مش واحدة

١. **الكارت كان بيرسم منطق سقف ميزانية بعد ما السقف اتشال.** `monthly_limit` بقى الرصيد
   الابتدائي في migration 20260816010000، بس الكارت فضل بيعرض `المصروف ÷ monthly_limit`
   كشريط تقدّم و"٪ من الميزانية" وسطر "الميزانية الشهرية: X". عميل رصيده الابتدائي ٥٠٠٠
   ودخله ١٠٠٠٠ كان بيشوف الشريط متملي وأحمر وهو معاه فلوس — النسبة ماكانتش غلط، كانت
   بتجاوب على سؤال مالوش وجود.

٢. **الرصيد كان بيتحسب من أول الدورة.** `zad_budget_state_legacy` بتجمع من `cycle_start`،
   فعميل نزّل التطبيق يوم ١٦، عدّ اللي معاه ولقاه ١٠٠٠ وكتبه، كان بياخد ١٠٠٠ ناقص خمستاشر
   يوم مصاريف **مطروحة من الـ١٠٠٠ دي أصلاً**. كل صف منهم كان بيتخصم مرتين.

٣. **الإشعار المقروء كان بيتكتب من غير موافقة.** ثقة ≥٠.٩ كانت بتخلي
   `handleNotificationIngest` يكتب المعاملة على طول والكارت يتحرك لوحده؛ الغامض بس هو
   اللي كان بيتسأل عنه. العميل طلب العكس.

### اللي اتعمل
- **`20260816120000`** — عمود `zad_users.balance_anchored_at`، و`zad_budget_state` بقت
  تجمع `spent`/`income`/`unverified_count` من عنده بدل `cycle_start`. مفيش حد أعلى عن قصد:
  الرصيد "اللي معايا" مابيتصفّرش مع دورة جديدة، الراتب اللي بينزل بيزوّده. `null` بيرجّع
  السلوك القديم بالحرف، فحسابات ما صرّحتش برصيد بعد المهاجرة ما اتغيّرتش. `by_category`
  و`cash_on_hand` سايبينهم على الدورة عن قصد — بيجاوبوا سؤال تاني، وإعادة تأسيسهم على
  نقطة اتحطت الصبح كانت هتفضّي شاشة محدش طلب تغييرها.
- **`BudgetMath`** — `anchoredAt: Instant?` اختياري على `spentInCycle`/`incomeInCycle`/
  `balanceInCycle`/`remainingInCycle`/`unverifiedCountInCycle`. المقارنة **بالثانية مش
  باليوم**: عميل ثبّت رصيده ٢ الضهر لازم يستبعد مصروف ١٠ الصبح من نفس اليوم. و`txInstant`
  بتقرا `+00:00` بتاعة Postgres، مش `Z` بس — من غير كده كل صف جاي من السيرفر كان هيسقط
  لبداية اليوم ويفقد الساعة اللي الفلتر كله قايم عليها.
- **`ZadCardHero`** — اتشال منه الشريطين والشرايح وسطر "الميزانية الشهرية". فضل: الرقم
  (`الرصيد`، من غير خصم المحجوز)، علامة `≈`، والضغطة الطويلة اللي بتفتح "آخر التغييرات".
  الرقم والرمز في `Text` منفصلين بمحاذاة خط القاعدة — الرمز على 44sp كان بيقص رصيد من ٦
  خانات على شاشة ٣٦٠dp.
- **`?job=confirm_transaction`** في `zad-telegram-bot` — بياخد معاملة مقروءة من زاد-برين،
  بيحط صف في `telegram_pending_writes` وبيبعت "جالك كذا؟"/"صرفت كذا؟" بنفس زرار التأكيد.
  الكتابة نفسها لسه في فرع `callback_query` بس، المكان الوحيد في البوت اللي كتب فلوس أصلاً.
  حساب مش مربوط بتيليجرام بياخد سؤال في `zad_insights` — مش تدهور، ده النص التاني من نفس
  القناة.
- **`UnifiedBankListener`** — المسار المحلي كله (`BankTransactionApplier` ←
  `BalanceAnchor.reconcile` ← إشعارات الراتب/الاشتراك) اتشال. السيرفر مش موصول = الطابور،
  مش كتابة محلية: تأخير أرخص من رقم اتغيّر لوحده والعميل ما يعرفش ليه.
- **`20260816130000`** — حالة `awaiting_confirmation` في `zad_notification_ingest_events`.
  `ambiguous` كانت هتبقى كدبة: دي إشعارات اتقرت تمام، والخلط بين "ما قدرناش نقراها"
  و"قريناها ومستنيين العميل" بيضيّع الفرق اللي الجدول ده موجود عشانه.

### مش متغطّى
التحويلات (سحب ATM) لسه بتتكتب على طول من غير سؤال — بطاقة ← كاش مابيغيّرش الرصيد، فمفيش
رقم للعميل يوافق عليه، و`log_transaction` بيرفض النوع ده أصلاً (`validators.ts`).

### التحقق
مفيش Android SDK ولا Deno في البيئة دي، فمفيش build أو `deno check` اتشغّل محلياً. اللي
...
## بوابة Home — إعادة بناء `HomeScreen.kt` (٢٠٢٦-٠٩-٠٣)

نُفذت مواصفة `UI_ARCHITECTURE_SPEC.md` §2.1/§4.1 بالكامل — تفاصيل التنفيذ والتحقق الفعلي
في سجل التنفيذ §6 بنفس الملف. ملخص: نظام تصميم فاخر جديد (`ZadHomeLuxe.kt`)، هيدر البوت
بقى فيه كاميرا حقيقية + شارة "عقل زاد نشط"، كارت مصروف أسبوعي بيزير جديد (كان مستورد
بدون استدعاء)، منتقي أقسام سريع جديد بيوصل للبوابة الصح فعليًا، كارت الصيدلية بقى فيه
"الجرعة التالية" حقيقية بزرار تسجيل، ويدجت التسبيحة بقى فيه تقدم تحدي العائلة الحقيقي،
وكارت النواقص بقى فيه "أضف للسلة" حقيقي. زيرو StateFlow أو route اتشال — الكروت التانوية
(اشتراكات/اختصارات/تليجرام/بريميوم/ملخص العقل) نقلت وراء إفصاح "عرض المزيد" حقيقي بدل ما
تفضل كلها ظاهرة دايمًا. البيئة دي معندهاش Android SDK أصلاً (خلاف ملاحظة `CLAUDE.md`) —
اتثبّت محليًا من الصفر، والتحقق الفعلي (`compileDebugKotlin` + `clean assembleDebug`)
نجح بعدها.

## تكرار الإشعار البنكي عبر المصادر (٢٠٢٦-٠٩-١٣)

الدفعة الواحدة كانت ممكن تتقيد ٢ أو ٣ مرات: البنك وInstaPay وSMS بيبعتوا ٣ إشعارات بصياغات
مختلفة، والطبقة الضبابية (`20260829020000`) محتاجة كلمة تاجر مشتركة فمابتمسكهمش، ودالة
الحسم ماكانتش بتدوّر على تكرار أصلًا. الإصلاح (`20260913213000_cross_source_duplicate_proposals.sql`):

- **وقت الاستلام** (`handleNotificationIngest` + `pickDuplicateProposalSibling` في `shared.ts`):
  اقتراح بنفس المبلغ خلال ١٥ دقيقة من اقتراح تاني (أقدمهم هو الأصل، واتجاه معروف ومختلف =
  مش تكرار) بيتعلّم `duplicate_of_proposal_id`، وسؤاله في تيليجرام والتطبيق بيبقى "هل دي نفس
  المعاملة؟" بزرارين `duplicate`/`separate`.
- **وقت الحسم** (`zad_resolve_transaction_proposal_impl`): قبل أي قيد، لو فيه اقتراح متقيد
  (معاملته لسه موجودة) أو معاملة من غير اقتراح بنفس المبلغ ونفس الاتجاه خلال ±١٥ دقيقة من وقت
  الإشعار → `duplicate_suspected` ومفيش قيد. قفل استشاري على مستوى العميل ضد ضغطتين متزامنتين.
  `duplicate` → حالة جديدة `merged` (مش `rejected`، عشان إحصائيات دقة التنبؤ). رفض/دمج أصل
  بينقل أبناءه لأصله أو بيرجعهم للسؤال العادي.
- التكرار مابيتشالش لوحده أبدًا — نفس مبدأ `allow_duplicate` في `log_transaction`.

التحقق: الميجريشن اتطبق على Postgres 16 محلي (docker) فوق `20260820004901` + `20260902020000`
(اتأكد إن نسخة الدالة المنشورة مطابقة للريبو بـmd5 قبل البناء عليها)، و٢٨ سيناريو SQL عدّوا —
منهم باج حقيقي اتمسك واتصلح: معاملة اتقيدت من اقتراح قديم created_at بتاعها وقت الضغطة، فكانت
بتبان توأم غلط لإشعار جديد. `deno test`: zad-brain 236، zad-telegram-bot 91، schema contract 1 —
كلهم عدّوا. `assembleDebug` → `BUILD SUCCESSFUL`.

ملاحظة مفتوحة مش متصلحة هنا: الطبقة الضبابية بتكتب `status: "duplicate_fuzzy"` على
`zad_notification_ingest_events`، والقيمة دي **مش** في `zad_notification_ingest_events_status_check`
— التحديث بيفشل بصمت (الخطأ مابيتقراش). غالبًا ده سبب إن العدّاد صفر في الداتابيز.
