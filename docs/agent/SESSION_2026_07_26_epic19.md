...
> ✅ **الجلسة دي مقفولة — Epic 1+4 (Tasks 19-24) خلص بالكامل.** الملف ده بقى أرشيف
> مرجعي، مش حالة شغل مفتوحة. الجلسة الجاية شغالة على **Epic 2 (إعادة تنظيم شاشة "ذكاء
> زاد")** — مفيش ملف تخطيط ليها لسه (`EPIC_1_4.md`/`PRODUCT_PLAN.md` بتغطوا Epic 1+4
> بس)، فأول خطوة الجلسة الجاية هي تحديد نطاق Epic 2 مع المستخدم، مش قراءة سبيسفيكيشن
> جاهزة. لقايات `docs/agent/AUDIT.md` (خصوصاً `ZadAlertRouter` المعزول تماماً،
> `detectSubscriptions()` بتكتب من غير تأكيد، والكود الميت `AssistantScreen.kt`/
> `ZadScreens.kt`) لسه مطروحة وممكن تتقاطع مع إعادة تنظيم شاشة الذكاء — يستاهل تتراجع
> قبل البدء.
>
> ملف حالة الجلسة دي بالذات (تاريخياً، وقت ما كانت شغالة). الصق الرابط ده أو محتواه في
> أول شات جديد عشان أرجع أعرف إحنا وصلنا فين. غير `SESSION_2026_07_26.md` (جلسة سابقة،
> Phase 0-5، شغل تاني خالص — لوحة العيلة والتنبيهات، خلصت قبل الجلسة دي).
...
- توضيح مطلوب قبل التنفيذ: "مقترحات قائمة التسوق" في الكود فيها أكتر من مسار (مش
  مسار واحد حاسم). المستخدم اختار: الاتنين (`InventoryFlowEngine.autoReplenish` +
  `ZadAiRepository.suggestGroceries`).
- `InventoryFlowEngine.isStagnant(context, item)` — راكد = مفيش نقص كمية ٣٠ يوم ومفيش
  عينة استهلاك خالص. المرجع الزمني هو آخر نشاط (شراء أو استهلاك) مش تاريخ الإضافة وحده
  — عشان صنف بيتجدد شراؤه بانتظام (فاتورة بتزوّد كميته) ميتصنفش راكد غلط. لو فيه أي
  عينة استهلاك خالص (حتى قديمة)، مش راكد أبداً — مطابق لمفهوم `rate_known` في Task 18.
- `autoReplenish()`: الأصناف الراكدة مستبعدة من التغذية التلقائية لقائمة التسوق.
- `fetchGrocerySuggestions()`: الأصناف الراكدة مش بتتبعت لل AI ضمن "المخزون الحالي" —
  فمقترحات الشراء الذكية ماتقترحش "اشتري منها كمان" لحاجة المستخدم أصلاً مش بيستخدمها.
- `generateUrgentRecipes()` (سياق الشيف): الأصناف الراكدة بقت بتتحط أول القايمة (قبل
  هتخلص/تنتهي)، ونافذة انتهاء الصلاحية اتوسّعت من يومين لـ ٥ أيام (مطابق للنص الحرفي).
  فرّقت الرسالة المبعوتة لل AI بين "راكد" و"هيخلص قريب" (سببين مختلفين تماماً).
- **فجوة موثّقة، مش اتصلحت**: التوثيق بيقول "أصناف هتنتهي خلال ٥ أيام بتوصل للعقل عن
  طريق الـ snapshot أصلاً" — ده غلط فعلياً، `zad-brain`'s `buildSnapshot()` بيقرا عمود
  `expiry_date` بس مايستخدموش خالص في أي حقل بيرجعه. مش اتصلح لأن "سياق الشيف" في
  الكود الفعلي (`generateUrgentRecipes`) بيقرا من `_inventory.value` مباشرة، مش من
  snapshot زاد-برين أصلاً — إصلاح الـ snapshot مكانش هيأثر على سياق الشيف حقيقي.
- UI: `UrgentRecipeCard` بقى يفرّق العنوان (`items_stagnant_hint` مقابل
  `items_expiring_soon_hint`) لو كل المحفّزات راكدة بس — عشان "هيخلص قريب" ماتتقالش
  غلط على صنف راكد مش هيتلف بكرة. ترجمات جديدة في الـ 4 ملفات locale (default/ar-SA/ar-EG/tr).
- اختبارات جديدة: `InventoryFlowEngineTest` (+5 حالات isStagnant). 94 اختبار إجمالي، 0 فشل
  (`testDebugUnitTest` اتأكد لوحده مرتين).
- **`lintDebug`/`assembleDebug` ماتمّوش هذه الجلسة** — الـ container فيه كذا Claude Code
  session شغالة بارالل (وصلت ٨ في وقت من الأوقات، اتأكد بـ `ps aux`)، والـ Gradle daemon
  (حتى `--no-daemon`) اتقتل بصمت (SIGKILL، مفيش stack trace) من ضغط الذاكرة **٥ مرات
