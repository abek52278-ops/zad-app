...
Default to terse, high-signal replies once a plan is executing — state what changed and why, skip preamble and restating the request back. Full explanations are for genuinely new architectural decisions, not routine edits. Never fabricate build/test results. **Deno IS installed** (`deno 2.9.5` at `~/.deno/bin/deno`, confirmed 2026-08-16 — the older note here said otherwise and was wrong), so `deno check` and `deno test` on `supabase/functions/` run locally and there is no excuse for pushing an edge-function change unverified. **An Android SDK is now present and Kotlin/Compose changes CAN be compiled locally** (confirmed 2026-09-03, superseding every earlier "no SDK" note in this file — the SDK directory's mtime is Aug 31, after the last "confirmed no SDK" check on 2026-08-16, so something in the environment/container image changed between those dates). `/home/codespace/android-sdk` exists with `build-tools 36.0.0` and `platforms/android-36`, wired up via `local.properties` (`sdk.dir=/home/codespace/android-sdk`), matching `app/build.gradle.kts`'s `compileSdk = 36`. **The `run-zad-app` skill's claim that an SDK is present was right all along** — the correction in this file's earlier revisions was itself wrong; trust the skill on this point now.
