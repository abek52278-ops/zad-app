...
3. **`ZadLuxe`** (`ui/theme/ZadHomeLuxe.kt`, `object ZadLuxe`) — short aliases **for the Heritage tokens themselves**, plus `squircle = RoundedCornerShape(20.dp)`. As of 2026-09-04 each value references its Heritage token (`val emerald = ZadForestEmerald`, etc.) instead of duplicating the hex, so `Color.kt` is the single source of truth and a palette change — dark mode above all — reaches ZadLuxe consumers too. **It is app-wide, not Home-only**: 23 files use it (Budget, Inventory, Pharmacy, Camera, Onboarding, Profile, Achievements, Shopping, the paywall…), plus `ZadListCard`'s defaults in `PremiumSurfaces.kt`, which are used 63 times across 18 files. Earlier revisions of this skill said "scoped to HomeScreen only, don't reach for it outside Home-screen code" and `ZadHomeLuxe.kt`'s own doc comment said the same; both were left behind by `UI_ARCHITECTURE_SPEC.md` §8, a 7-phase rollout of ZadLuxe across the whole app that completed 2026-09-04. Using it outside Home is normal and correct.

**Rule of thumb**: Cupertino Heritage is the authoritative token source (settled 2026-09-04) — new screen with no established local style → Heritage via `MaterialTheme`. Editing a file that already uses `ZadLuxe` → keep using `ZadLuxe`; it now resolves to the same Heritage tokens, so it is not a competing system and there is nothing to migrate. Editing an existing `ZadShell`/`ZadAnimatedLogo`-family file → match its `ZadV3` references (6 files; still a genuinely separate palette, and the only real migration left). Don't introduce a fourth system.
